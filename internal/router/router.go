...
func New(postSvc *service.PostService, authorSvc *service.AuthorService, categorySvc *service.CategoryService, commentSvc *service.CommentService, authSvc *service.AuthService, cfg *config.Config) http.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Route("/posts", func(r chi.Router) {
			...
			r.Get("/{id}", ph.GetByID)
			r.With(middleware.RequireAuth(cfg.JWTSecret)).Post("/", ph.Create)
			r.With(middleware.RequireAuth(cfg.JWTSecret)).Put("/{id}", ph.Update)
			r.With(middleware.RequireAuth(cfg.JWTSecret)).Delete("/{id}", ph.Delete)
		})
